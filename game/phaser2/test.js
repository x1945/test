...
function create() {
	...
	for (var i = 0; i < 500; i++) {
		var c = group.create(game.rnd.integerInRange(200, 1900), game.rnd
				.integerInRange(0, 1100), 'veggies', game.rnd.integerInRange(0,
				35));
		c.name = 'veg' + i;
	...
	for (var i = 0; i < 20; i++) {
		// Here we'll create some chillis which the player can pick-up. They are
		// still part of the same Group.
		var c = group.create(game.rnd.integerInRange(100, 770), game.rnd
				.integerInRange(0, 570), 'veggies', 17);
		c.body.immovable = true;
	}
